...
# Output features: (N x 25088) using conv+avgpool (no classifier)
...
OUT_FILE = r"E:\Single_frame_smile\data\Isornot\features\vgg16_matched_features.npz"
BATCH_SIZE = 16  # VGG feature is huge, reduce batch
NUM_WORKERS = 0
USE_AMP = True
...
class VGG16FeatureExtractor(torch.nn.Module):
    def __init__(self):
        ...
        self.avgpool = m.avgpool  # AdaptiveAvgPool2d((7,7))
...
def build_vgg16_feature_extractor(device: torch.device) -> torch.nn.Module:
    m = VGG16FeatureExtractor()
    m.eval()
...
def main():
    ...
    model = build_vgg16_feature_extractor(device)
