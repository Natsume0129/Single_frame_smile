...
def main() -> None:
    ...
    ax.grid(alpha=0.25)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "heatmap_area_over_threshold_075_over_time.png")
    plt.close(fig)
    ...
    ax.grid(alpha=0.25)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "heatmap_sum_over_threshold_075_over_time.png")
    plt.close(fig)

    print(f"[THRESHOLD_METRICS_075] Saved plots to: {OUT_DIR}")
